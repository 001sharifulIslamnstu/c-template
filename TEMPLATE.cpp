...
struct point_double{double x , y ; point_double (){}point_double ( double a , double b){x=a , y=b;}};
 ///Point for x , y ( double ) coordinate in 2D space

struct Node{int v ,w;Node() {}bool operator <(const Node &a) const{return w>a.w;} Node( int v , int w){v= v ,w= w ;}};
///Node for Dijkstra
namespace my{
...
namespace debug{
    ...
    template<typename T, typename ... Args> T sum(T a, Args ... args ) {return a+sum(args ...) ;}
      ...
      void printf(T a , Args ... args){cout<<a<<" "; print (args ...) ;}
     }
